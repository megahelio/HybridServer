...
public class HTMLController {
	 ...
	 * @param port
	 */
	public HTMLController(DaoHTML dao, List<ServerConfiguration> servers, int port) {
		this.dao = dao;
	...
	public HTTPResponse list(HTTPRequest request) {
		HTTPResponse response = new HTTPResponse();
		String fullList = "<html><body><h1>HTML List</h1><br>";
		fullList = fullList.concat("<h1>LocalHost</h1><br>");
		fullList = fullList.concat("<ul>");
		List<String> uuids = this.dao.listPages();
		for (String uuid : uuids) {

			fullList = fullList.concat(
					"<li><a href=http://localhost:" + port + "/html?uuid=" + uuid + ">" + uuid + "</a>" + "</li>");
		}
		fullList = fullList.concat("</ul>");
		...
		for (ServerConfiguration server : this.servers) {
			...
			if (!server.getName().equals("Down Server")) {
				...
				try {
					...
					System.out.println("URL SERVER: " + server.getWsdl());
					System.out.println(
							server.getHttpAddress() + "  -  " + server.getNamespace() + "  -  " + server.getService());
					QName name = new QName(server.getNamespace(), server.getService() + "ImplService");

					Service service = Service.create(url, name);

					HybridServerService ws = service.getPort(HybridServerService.class);
					fullList = fullList.concat("<h1>" + server.getName() + "</h1><br>");

					uuids = ws.listPagesHTML();
					fullList = fullList.concat("<ul>");
					for (String uuid : uuids) {

						fullList = fullList.concat("<li><a href=" + server.getHttpAddress() + "html?uuid=" + uuid + ">"
								+ uuid + "</a>" + "</li>");
					}
					fullList = fullList.concat("</ul>");
		...
		response.setContent(fullList + "</body></html>");
